...
- After TypeScript/plugin changes (not content/docs): `deno task check` (full output, no tail). Fix all errors before committing; run `deno fmt` to fix formatting. Does not run tests.
